...
import '../../domain/state/home_music_hall_state.dart';
...
class _SearchPanelState extends ConsumerState<SearchPanel> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      ref
          .read(homeMusicHallStateProvider.notifier)
          .ensureLoaded(source: s.searchSource);
    });
  }
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final isCurrent =
                                    ...
                                    playerState.currentSong?.source ==
                                        song.source;
                                final isPlaying =
  ...
  Widget _buildHomeContent(
    ...
    final musicHallState = ref.watch(homeMusicHallStateProvider);
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          _RecentlyPlayed(
            onTap: (song) {
              ref
                  ...
                  .playSong(
                    song,
                    quality: ref.read(settingsStateProvider).playbackQuality,
                  );
            },
          ),
          ...
          if (musicHallState.loading)
            const SizedBox(
              height: 140,
              child: Center(child: CircularProgressIndicator()),
            )
          else if (musicHallState.leaderboards.isEmpty)
            const SizedBox(height: 140, child: Center(child: Text('暂无排行榜数据')))
          else
            Padding(
              ...
              child: GridView.builder(
                ...
                itemCount: musicHallState.leaderboards.length,
                itemBuilder: (context, index) {
                  final item = musicHallState.leaderboards[index];
                  return _LeaderboardCard(
                    item: item,
                    source:
                        musicHallState.loadedSource ??
                        ref.read(settingsStateProvider).searchSource,
                  );
                },
              ),
            ),
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => LeaderboardDetailScreen(item: item, source: source),
          ),
        );
      },
      child: Card(
            ...
            builder: (context, constraints) {
              ...
              return Wrap(
                ...
                children:
                    songs.map((song) {
                      final isCurrent =
                          song.id == currentId && song.source == currentSource;
                      return GestureDetector(
                        onTap: () => onTap(song),
                        child: Container(
                          width: itemWidth,
